...
# NOTE: computing the above with "nn.KLDivLoss()" or "F.kl_div" does not work well, as its computed gradients seems
#       not to scale with scalar multiples at all (not even when multiplied with zero!)
...
def log_Logistic_256(x, mean, logvar, average=False, dim=None):
    '''Calculate log-likelihood of sample [x] under Logistic distribution(s) with mu=[mean], diag_var=exp[log_var].
    NOTES: [dim]=-1    summing / averaging over all but the first dimension
           [dim]=None  summing / averaging is done over all dimensions'''
    ...
    if dim is not None and dim==-1:
        ...
        dim = 1
    if average:
        return torch.mean(log_logist_256, dim) if dim is not None else torch.mean(log_logist_256)
    else:
        return torch.sum(log_logist_256, dim) if dim is not None else torch.sum(log_logist_256)
